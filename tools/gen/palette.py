...
PALETTE = {
    ...
    # Ground under a canopy. Its own keys rather than a shade of the meadow's,
    # because the difference that reads is not brightness alone: the meadow's
    # green leans yellow (hue 97) and shaded ground leans blue (120). Measured
    # against the turf it has to be told apart from - grass 107, this 86 - so
    # the wood announces itself before a single trunk is drawn. The forest
    # floor was leaf litter for a long time, which put a brown field at hue 37
    # next to a meadow at 101 and split every map in two.
    "FR":  ((0x38, 0x62, 0x38, 255), "forest floor"),
    "FRD": ((0x28, 0x49, 0x2b, 255), "forest floor shade"),
    "FRL": ((0x4e, 0x7e, 0x46, 255), "forest floor light"),
    ...
    # Bone and bleached linen. The pale tone on cloth used to be borrowed
    ...
    # Dust hanging in the air, and the only translucent colour besides the
    # ground shadow: a cloud you cannot see the grass through is a balloon.
...
VARIANTS = {
    ...
    # Three more people, and every one of them is a remap: a settlement reads
    # as a settlement because of who is standing in it, and at this size the
    # difference between one villager and the next is entirely what they wear.
    "guard": {
        "TU":  (0x5a, 0x66, 0x78, 255),   # a steel-grey coat over mail
        "TUS": (0x3c, 0x45, 0x54, 255),
        "TUL": (0x7d, 0x8b, 0x9e, 255),
        "PN":  (0x3a, 0x3f, 0x4a, 255),
        "PNS": (0x26, 0x2a, 0x33, 255),
        "HR":  (0x3a, 0x2e, 0x24, 255),
        "HRL": (0x55, 0x43, 0x33, 255),
    },
    "farmer": {
        "TU":  (0xb8, 0x9a, 0x5e, 255),   # undyed, and worn pale by the sun
        "TUS": (0x8c, 0x73, 0x42, 255),
        "TUL": (0xd6, 0xbc, 0x84, 255),
        "PN":  (0x5f, 0x4a, 0x33, 255),
        "PNS": (0x42, 0x32, 0x22, 255),
        "HR":  (0x7a, 0x5f, 0x30, 255),
        "HRL": (0xa2, 0x81, 0x46, 255),
    },
    "innkeeper": {
        "TU":  (0x8e, 0x3d, 0x44, 255),   # wine red, over a pale apron
        "TUS": (0x63, 0x28, 0x2e, 255),
        "TUL": (0xb2, 0x58, 0x5e, 255),
        "PN":  (0xc9, 0xc0, 0xa8, 255),
        "PNS": (0x9d, 0x94, 0x7e, 255),
    ...
    "cow": {
        # Red-brown over cream, with the horn keys left alone: a cow's horns
        # are the same ivory as a goat's and there is nothing to gain by
        # saying it twice.
        "AB":  (0x8e, 0x4f, 0x33, 255),
        "ABS": (0x66, 0x37, 0x24, 255),
        "ABL": (0xb4, 0x6f, 0x4c, 255),
        "AF":  (0x5c, 0x32, 0x20, 255),
        "AFS": (0x3f, 0x22, 0x16, 255),
        "AH":  (0x2c, 0x22, 0x1c, 255),
    },
    "deer": {
        # Tawny, and the rump left at the default pale - which is the one
        # place the palette already had the right answer waiting.
        "AB":  (0xa8, 0x7a, 0x4a, 255),
        "ABS": (0x7e, 0x59, 0x34, 255),
        "ABL": (0xc6, 0x99, 0x66, 255),
        "AF":  (0x6b, 0x4b, 0x2e, 255),
        "AFS": (0x4a, 0x33, 0x1f, 255),
        "AH":  (0x33, 0x27, 0x1c, 255),
    },
    "boar": {
